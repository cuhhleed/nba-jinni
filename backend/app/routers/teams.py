...
async def get_team_details(team_id: int, db: AsyncSession = Depends(get_db)):
    stmt = select(Team).where(Team.id == team_id).options(selectinload(Team.standing))
...
async def get_team_stats(team_id: int, db: AsyncSession = Depends(get_db)):
    ...
    season_average = await db.scalar(
        select(TeamSeasonAverage).where(
            TeamSeasonAverage.team_id == team_id,
            TeamSeasonAverage.season == current_season,
        )
    )
...
async def get_team_roster(team_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if not players:
        logger.warning(
            "team_empty_roster",
            message="Team has no players, but probably should.",
            team_id=team_id,
        )
...
async def get_team_games(team_id: int, db: AsyncSession = Depends(get_db)):
    ...
    recent = [
        GameWithTeamStats.model_validate(g) for g in recent_result.scalars().all()
    ]
