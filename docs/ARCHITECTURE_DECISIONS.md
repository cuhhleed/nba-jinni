...
### Amendment (2026-06-03) — replace `round: int` with `round_label: str | None`

**Context.** The original `round` integer (1–4) was derived from the NBA `gameLabel` string via `_parse_playoff_round` — an exact-match table over `("1st Round",)`, `("Conf. Semifinals", "Conference Semifinals")`, etc. The function's own comment acknowledged the brittleness ("must be validated against real schedule data before relying on it in production"). Production data validated the concern: 80 of 88 playoff games in the populated DB had `round = 0` (the parse-failure fallback). Frontend rendering then collapsed those rows to `"PLAYOFFS · Game X"` instead of the real series identity.

**Change.** Drop the parser. Store the raw NBA `gameLabel` on `playoff_game_metadata.round_label` (nullable text). The frontend renders the label verbatim — whatever the API returns is what the user sees, with a fall-back to `"Playoffs"` when the label is absent.

**Touch points** (all in the same PR):
- Model — `playoff_game_metadata.round` → `round_label: Optional[str]`.
- Migration — autogenerated: `DROP COLUMN round`, `ADD COLUMN round_label TEXT NULL`. Additive + drop; no `add_required_column` choreography needed because the new column is nullable.
- Ingestion — `ingest_playoff_schedule` writes `round_label = game["gameLabel"]` in both the `.values()` and `set_={...}` blocks; `_parse_playoff_round` is deleted.
- Backend schema — `PlayoffMetadata.round_label: Optional[str]`.
- Frontend — `PlayoffMetadata.round_label: string | null`; `PlayoffSeriesBanner` renders it directly; `playoffRoundLabel` util removed.

**Healing.** The 80 misparsed rows recover automatically on the next `ingest_playoff_schedule` run — the upsert's `set_={...}` already covers `round_label`.

**Trade-off accepted.** We lose the ability to filter by an integer round (`WHERE round = 4`). No call sites in the codebase used this, and progression can still be derived from chronological `tipoff_at` ordering. If integer filtering becomes necessary later, it can be re-derived from `round_label` at query time without re-introducing the lossy ingestion-time parser.
